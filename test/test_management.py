...
class TestManagement(BaseTestCase):
    ...
    async def test_basic_sanity(self):
        ...
        core_client = httpmq.APIClient(base_url=get_unittest_httpmq_mgmt_api_url())
        mgmt_client = httpmq.ManagementClient(api_client=core_client)
        await mgmt_client.ready(context=httpmq.RequestContext())

        another_client = httpmq.APIClient(base_url="http://127.0.0.1:17881")
        another_mgmt_client = httpmq.ManagementClient(api_client=another_client)
        with self.assertRaises(aiohttp.client_exceptions.ClientConnectorError):
            await another_mgmt_client.ready(context=httpmq.RequestContext())
    ...
    async def test_stream_management(self):
        ...
        new_limits = httpmq.ManagementJSStreamLimits(max_age=new_max_age)
        context = httpmq.RequestContext()
        self.assertEqual(
            context.request_id,
    ...
    async def test_consumer_management(self):
        ...
        stream_1_param = httpmq.ManagementJSStreamParam(
            name=stream_1, subjects=subjects_1
        )
        context = httpmq.RequestContext()
        self.assertEqual(
            context.request_id,
        ...
        with self.assertRaises(httpmq.HttpmqAPIError):
            await mgmt_client.create_consumer_for_stream(
                stream=stream_1, params=consumer_param, context=httpmq.RequestContext()
            )
        ...
        with self.assertRaises(httpmq.HttpmqAPIError):
            await mgmt_client.get_stream(
                stream=stream_1, context=httpmq.RequestContext()
            )
